<%@page import="com.upem.models.Emprunt"%>
...
Vector<Emprunt>em = (Vector<Emprunt>)request.getAttribute("Emp");
Vector<Produit>vec = (Vector<Produit>)request.getAttribute("prod");
Vector<User>users = (Vector<User>)request.getAttribute("us");
%>
<meta charset="utf-8">
				...
				<div class="card-body">
					<table id="example1" class="table table-bordered table-striped">
                	  ...
                	  for(Emprunt e : em){  
                		  ...
                		  String usr="";
                		  for(Produit p : vec){
                			  if(p.getId()==e.getIdProduit())
                				  produit = p.getDesig();
                		  }
                		  for(User x : users){
                			  if(x.getIdUser()==e.getUser())
                				  usr =x.getLogin();
                		  }
                		  
                  %>
							<tr>
								<td><%= e.getIdEmprunt() %></td>
								<td><%= produit %></td>
								<td><%=e.getDateEmprunt() %></td>
								<td><%=e.getDateRestitution() %></td>
								<td><%=usr %></td>
								<td>
									<form action="Edite" name="forma">
										<input type="hidden" name="ed" value="<%=e.getIdEmprunt() %>" />
										<input type="submit" class="btn btn-info" value="Edite">
